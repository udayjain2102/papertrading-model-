...
========
# Audit — 2026-07-16: questioning everything against the original goal

> **Historical record. Read for the reasoning, not the facts.** Written before
> the 2026-07-17 cleanup and the later dead-code deletions; several files it
> names (`runner.py`, `executor.py`, `agent.py`, `compare.py`) no longer exist.
> For how the system works now, see [`ARCHITECTURE.md`](ARCHITECTURE.md).
> The "road to real money" ladder at the bottom is still the standing plan.
>>>>>>>> origin/main:docs/archive/AUDIT-2026-07-16.md
