...
def _equity_svg(net: pd.Series, width: int = 900, height: int = 260) -> str:
    if len(net) == 0:
    ...
    area = (
        f"{px(0):.1f},{height - pad:.1f} " + pts + f" {px(n - 1):.1f},{height - pad:.1f}"
    )
    final = ys[-1]
    stroke = "var(--up)" if final >= 1.0 else "var(--down)"
    baseline = (
        f"<line x1='{pad}' y1='{base_y:.1f}' x2='{width - pad}' y2='{base_y:.1f}' "
        f"class='baseline'/>" if base_y is not None else ""
    )
    return f"""<svg viewBox="0 0 {width} {height}" class="equity" preserveAspectRatio="xMidYMid meet">
  <polygon points="{area}" fill="{stroke}" opacity="0.10"/>
  {baseline}
  <polyline points="{pts}" fill="none" stroke="{stroke}" stroke-width="2"/>
  <text x="{pad}" y="16" class="svglabel">equity {equity.iloc[-1]:.3f}× ({_pct(final - 1)})</text>
</svg>"""
...
def _compare_table(df: pd.DataFrame, current: str) -> str:
    ...
    for _, r in df.iterrows():
        ...
        rows.append(
            f"<tr class='{cls}'><td class='mono'>{escape(str(r['run_id']))}</td>"
...
padding:9px 12px;color:var(--muted);font-weight:600;white-space:nowrap;border-bottom:1px solid var(--line)}
...
.baseline{stroke:var(--line);stroke-dasharray:4 4}
